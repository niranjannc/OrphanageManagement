...
<meta charset = "utf-8" />
<meta name = "description" content = "Donor Form" />
<meta name = "keywords" content = "Donor form" />
<meta name = "viewport" content = "width=device-width; initial-scale=1.0" />

<title>Donor/Trustee Form</title>
...
<?php
$fname=$lname=$gender=$bdate=$addlocation=$addcity=$adddistrict=$addstate=$phone=$photo=$photoerr=$role=$occupation="";
...
if ( $_SERVER["REQUEST_METHOD"] == "POST" ) {
	...
	$occupation = testData( $_POST["occupation"] );
	...
	if($photoerr == "") {
		try {
			...
			$sql1 = "SELECT count(*) as count from donor";
			...
			$sql2 = "insert into donor values('$orgid','$did', '$fname', '$lname', '$gender', '$bdate', '$addlocation', '$addcity', '$adddistrict', '$addstate', '$phone', '$occupation', null, '$role', '$photo')";
			...
			if($status === true) {
				echo "<script>alert(\"Next Step : DOnation Procedure\");</script>";
...
<li><button type = "button" >Add Donors/Trustees</button></li>
<li><button type = "button" onclick = "window.location.href = 'donationlist.php'">Donation List</button></li>
</ul>
</div>

<h1>FORM TO ADD DONOR/TRUSTEE</h1>

<form method = "POST" action = "<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" enctype = "multipart/form-data" >
	
	<div class = "formcontents">
	
		<label for = "orgid" >Organisation</label><br />
		...
		<label for = "role" >Select any one</label><span> * </span><br />
		<input type = "radio" id = "role" name = "role" value = "donor" <?php if (isset($role) && $role == "role") echo "checked" ?> /><label>Donor</label>		
		<input type = "radio" id = "role" name = "role" value = "trustee" <?php if (isset($role) && $role== "trustee") echo "checked" ?> /><label>Trustee</label><br /><br /><br />
	...
	<div class = "formcontents">
		...
		<input type = "text" id = "occupation" name = "occupation" placeholder = "Occupation" value = "<?php echo $occupation;?>" required autocomplete = "off" /><span> * </span><br /><br />
